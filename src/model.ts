...
import { BoxGeometry, BufferAttribute, DoubleSide, FrontSide, Group, Mesh, MeshLambertMaterial, Object3D, Texture, Vector2 } from "three";
...
export class SkinObject extends Group {
	...
	constructor(texture: Texture) {
		...
		const layer1Material = new MeshLambertMaterial({
			map: texture,
			side: FrontSide
		});
		const layer2Material = new MeshLambertMaterial({
			map: texture,
			side: DoubleSide,
			transparent: true,
			alphaTest: 1e-5
...
export class CapeObject extends Group {
	...
	constructor(texture: Texture) {
		...
		const capeMaterial = new MeshLambertMaterial({
			map: texture,
			side: DoubleSide,
			transparent: true,
			alphaTest: 1e-5
...
export class ElytraObject extends Group {
	...
	constructor(texture: Texture) {
		...
		const elytraMaterial = new MeshLambertMaterial({
			map: texture,
			side: DoubleSide,
			transparent: true,
			alphaTest: 1e-5
